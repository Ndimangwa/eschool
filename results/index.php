...
<div class="module-title ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content module-dimension">
	<div class="ui-sys-panel-header ui-widget-header">Reports Manager</div>
	<div class="ui-sys-panel-body" style="font-size: 0.9em; position: relative;">
		<form class="form-module pure-form pure-form-aligned form1" method="POST" action="<?= $thispage ?>">
			<table>
				<tr>
					<td><label><input id="managestudent_academic_progress_report" type="radio" name="page" value="managestudent_academic_progress_report"/></label></td>
					<td><label style="font-style: italic; font-weight: normal;" for="managestudent_academic_progress_report">Student Academic Progress Report</label></td>
				</tr>
				<tr>
					<td><label><input id="managestudent_overall_summary_results" type="radio" name="page" value="managestudent_overall_summary_results"/></label></td>
					<td><label style="font-style: italic; font-weight: normal;" for="managestudent_overall_summary_results">Overall Summary Results</label></td>
				</tr>
				<tr>
					<td><label><input id="another_reports" type="radio" name="page" value="another_reports"/></label></td>
					<td><label style="font-style: italic; font-weight: normal;" for="another_reports">Other Academic Reports</label></td>
				</tr>
			</table>
			<div class="ui-sys-error-message"></div>
			<div class="ui-sys-inline-controls-right">
				<input class="__add_record" type="submit" value="Query Reports"/>
			</div>
		</form>
	</div>
	<div class="ui-sys-panel-footer"></div>
...
<?php 
	/*BEGIN Overall Summary Results*/if ($page == "managestudent_overall_summary_results" && isset($_REQUEST['print_report'])&& isset($_REQUEST['id']) && isset($_REQUEST['paperId']) && Authorize::isAllowable($config, "managestudent_overall_summary_results", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		var_dump($_REQUEST);
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">OVERALL SUMMARY RESULTS</div>
			<div class="ui-sys-panel-body"></div>
			<div class="ui-sys-panel-footer"><a href="<?= $thispage ?>?page=managestudent_overall_summary_results">Print Another Report</a></div>
		</div>
<?php
		mysql_close($conn);
	} else if ($page == "managestudent_overall_summary_results" && isset($_REQUEST['select_paper']) && isset($_REQUEST['id']) && Authorize::isAllowable($config, "managestudent_overall_summary_results", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">OVERALL SUMMARY RESULTS</div>
			<div class="ui-sys-panel-body ui-sys-data-capture">
				<form id="form1" class="pure-form" method="POST">
					<input type="hidden" name="page" value="managestudent_overall_summary_results"/>
...
?>
			...
			<div class="ui-sys-panel-footer"><a href="<?= $thispage ?>?page=managestudent_overall_summary_results">Cancel the Operation</a></div>
		</div>
<script type="text/javascript">
(function($)	{
	$(function()	{
...
<?php
		mysql_close($conn);
	} else if ($page == "managestudent_overall_summary_results" && isset($_REQUEST['list']) && Authorize::isAllowable($config, "managestudent_overall_summary_results", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
...
<div class="ui-sys-search-results">
	...
	<table class="pure-table ui-sys-table-search-results" style="width: 100%; font-size: 12px;">
		<thead>
			<tr>
				<th>S/N</th>
				<th></th>
...
?>				
		...
		<a title="Get Overall Summary Results" href="<?= $thispage ?>?page=managestudent_overall_summary_results&id=<?= $group1->getGroupId() ?>&select_paper=true" class="ui-sys-search-results-controls"><img alt="DAT" src="../sysimage/buttondetail.png"/></a>
...
?>
		<div class="ui-state-error">
			There were problems in fetching Groups <br/>
...
<?php
		mysql_close($conn);
	} else if ($page == "managestudent_overall_summary_results" && Authorize::isAllowable($config, "managestudent_overall_summary_results", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">OVERALL SUMMARY RESULTS</div>
			<div class="ui-sys-panel-body ui-sys-data-capture">
				<form id="form1" class="pure-form pure-form-aligned" action="<?= $thispage ?>" method="POST">
					<input type="hidden" name="page" value="managestudent_overall_summary_results"/>
...
?>
					...
					<div class="pure-controls ui-sys-inline-controls-right">
						<input type="button" value="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Next&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" id="__add_record"/>
					</div>
				</form>
			</div>
			<div class="ui-sys-panel-footer"></div>
...
<?php
		mysql_close($conn);
	}/*END Overall Summary Results*/ else /*BEGIN Academic Progress Report*/if ($page == "managestudent_academic_progress_report" && isset($_REQUEST['print_report']) && isset($_REQUEST['id']) && Authorize::isAllowable($config, "managestudent_academic_progress_report", "normal", "setlog", "-1", "-1"))	{
		...
		try {
			...
			$paper1->putData('uhuruNaUmoja', '../data/profile/nembo_ya_taifa.jpg');
			$paper1->putData('mriLogo', '../data/profile/logo/'.$profile1->getLogo());
			$paper1->putData('documentSeal', '../data/profile/mri_seal.jpg');
			$paper1->putData('resultsFolder', '../data/results/');
			$window1 = Document::getStudentAcademicProgressReport($database, $conn, $profile1, $student1, $paper1, 0);
		} catch (Exception $e)	{
			$promise1->setReason($e->getMessage());
			$promise1->setPromise(false);
		}
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">Student Academic Progress Report <br/> For <i><?= $ulogin1->getFullname() ?></i></div>
			<div class="ui-sys-panel-body" style="background-color: black; overflow-x: scroll; padding: 5px;">
<?php 
	if ($promise1->isPromising())	{
?>
		<div class="ui-sys-float-right"><a href="#" class="menu-icon menu-icon-print" data-content-container="__view_student_progress_report"><img title="Click Here to Print" alt="PRT" src="../sysimage/print.png"/></a><a href="#" class="menu-icon" data-content-container="__examination_signing_sheet"><img title="Click to Dowload Pdf File" alt="PDF" src="../sysimage/pdf.png"/></a></div>
...
?>
		<div class="ui-state-error">
			There were problems in generating Student Academic Progress Report <br/>
			Details : <?= $promise1->getReason() ?>
		</div>
<?php
	}
?>			
			...
			<div class="ui-sys-panel-footer"><a href="<?= $thispage ?>">Go to Home Page</a></div>
		</div>
<?php
		mysql_close($conn);
	} else if ($page == "managestudent_academic_progress_report" && isset($_REQUEST['select_paper']) && Authorize::isAllowable($config, "managestudent_academic_progress_report", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		...
		$fullname = "Unknown";
		try {
			$studentId = Student::getStudentIdFromRegistrationNumber($database, $conn, $registrationNumber);
...
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">Student Academic Progress Report <br/> For <i><?= $fullname ?></i></div>
			<div class="ui-sys-panel-body ui-sys-data-capture ui-sys-inline-controls-center">
<?php 
	if ($promise1->isPromising())	{
?>
				<form id="form1" class="pure-form" method="POST">
					<input type="hidden" name="page" value="managestudent_academic_progress_report"/>
					<input type="hidden" name="id" value="<?= $student1->getStudentId() ?>"/>
					<input type="hidden" name="print_report"/>
					<div class="ui-sys-warning">
						<b>Please Note</b> : The type of paper you are selecting will affect both presentatio view and printing view.
					</div>
					<label>Select Printing Paper <select name="paperId" validate="true" validate_control="select" validate_expression="select" validate_message="Kindly Select a Paper">
...
?>
		<div class="ui-state-error">
			There were problems in pulling student's record <br/>
			Details : <?= $promise1->getReason() ?>
		</div>
<?php
	}
?>
			...
			<div class="ui-sys-panel-footer"><a href="<?= $thispage ?>?page=managestudent_academic_progress_report">Back to Start</a></div>
		</div>
<script type="text/javascript">
(function($)	{
	$(function()	{
...
<?php
		mysql_close($conn);
	} else if ($page == "managestudent_academic_progress_report" && Authorize::isAllowable($config, "managestudent_academic_progress_report", "normal", "setlog", "-1", "-1"))	{
?>
		<div class="ui-sys-panel-container ui-sys-panel ui-widget ui-widget-content">
			<div class="ui-sys-panel-header ui-widget-header">Student Academic Progress Report</div>
			<div class="ui-sys-panel-body ui-sys-data-capture ui-sys-inline-controls-center">
				<form id="form1" class="pure-form" method="POST">
					<input type="hidden" name="page" value="managestudent_academic_progress_report"/>
					<input type="hidden" name="select_paper" value="true"/>
					<div><label><span title="Enter the Registration Number of the student you want to pull the Information">Student's Registration Number</span> : <input type="text" name="registrationNumber" id="registrationNumber" size="32" required pattern="<?= $exprRegistrationNumber ?>" validate="true" validate_control="text" validate_expression="<?= $exprRegistrationNumber ?>" validate_message="<?= $msgRegistrationNumber ?>"/></label></div>
					<div>
						<span class="ui-sys-error-message" id="perror"></span>
					</div>
					<div class="ui-sys-inline-controls-right">
						<input type="button" id="__add_record" value="Continue"/>
					</div>
				</form>
			</div>
			<div class="ui-sys-panel-footer"></div>
...
<?php
	}/*END Academic Progress Report*/ else /*BEGIN Results Synchronization*/if ($page == "manageresults_synchronize" && isset($_REQUEST['report']) && Authorize::isAllowable($config, "manageresults_synchronize", "normal", "setlog", "-1", "-1")) {
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		$promise1 = new Promise();
		$promise1->setPromise(true);
		$enableUpdate = false;
...
<?php 
	foreach ($list as $groupId)	{
		...
		if ($group1->canViewCompiledSummaryResults())	{
			$folder = "__add_now_";
			$mode = Results::$__RESULTS_MODE;
			$key = $class.$id.$folder.$mode.$hashText;
			$key = md5($key);
			$classstring = strtolower($class);
			$nextpage = $thispage."?page=managesummaryresults&class=$class&id=$id&folder=$folder&mode=$mode&caption=$caption&key=$key";
			$actionIcons .= "<a href=\"$nextpage\" title=\"Compiled Summary Results\" class=\"ui-sys-icon-80-by-16 icon-summary-results\"></a>";
		}
		$semester = "All";
...
<?php
		mysql_close($conn);
	}/*END Results View Services*/ else /*BEGIN Results View Services*/if ($page == "managesummaryresults" && isset($_REQUEST['service_view_results']) && isset($_REQUEST['paperId']) && isset($_REQUEST['class']) && isset($_REQUEST['id']) && isset($_REQUEST['folder']) && isset($_REQUEST['mode']) && isset($_REQUEST['caption']) && isset($_REQUEST['key']) && Authorize::isAllowable($config, "manage".strtolower($_REQUEST['class'])."_detail", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		$promise1 = new Promise();
		$promise1->setPromise(true);
		$captionText = $_REQUEST['caption'];
		...
		try {
			...
			$window1 = $object1->getMyExaminationResultsSheetUI($profile1, $paper1, $dataFolder, $horizontalScale, $controlFlags);
		} catch (Exception $e)	{
			$promise1->setReason($e->getMessage());
			$promise1->setPromise(false);
		}
...
<?php
		mysql_close($conn);
	} else if ($page == "managesummaryresults" && isset($_REQUEST['class']) && isset($_REQUEST['id']) && isset($_REQUEST['folder']) && isset($_REQUEST['mode']) && isset($_REQUEST['caption']) && isset($_REQUEST['key']) && Authorize::isAllowable($config, "manage".strtolower($_REQUEST['class'])."_detail", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		$promise1 = new Promise();
		$promise1->setPromise(true);
		$captionText = $_REQUEST['caption'];
...
?>
			<div>
		<form id="form1" class="pure-form ui-sys-data-capture" method="POST" action="<?= $thispage ?>">
			<input type="hidden" name="page" value="managesummaryresults"/>
			<input type="hidden" name="service_view_results" value="true"/>
			<input type="hidden" name="class" value="<?= $_REQUEST['class'] ?>"/>
			<input type="hidden" name="id" value="<?= $_REQUEST['id'] ?>"/>
			<input type="hidden" name="folder" value="<?= $_REQUEST['folder'] ?>"/>
...
<?php
		mysql_close($conn);
	}/*END Results Upload Services*/else /*BEGIN Nullification*/if ($page == "managenullificationservices" && isset($_REQUEST['report']) && isset($_REQUEST['rndx']) && isset($_REQUEST['class']) && isset($_REQUEST['id']) && isset($_REQUEST['caption']) && isset($_REQUEST['folder']) && isset($_REQUEST['key']) && Authorize::isAllowable($config, "manage".strtolower($_REQUEST['class'])."_edit", "normal", "setlog", "-1", "-1") && Authorize::isAllowable($config, "manageresults_nulify", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		$promise1 = new Promise();
		$promise1->setPromise(true);
		$object1 = null;
		...
		try {
			...
			$object1 = ClassRegistry::getObjectReference($database, $conn, $_REQUEST['class'], $_REQUEST['id']);
			if (is_null($object1)) Object::shootException("Class could not be found in the Registry");
			if (trim($_REQUEST['rndx']) != $object1->getExtraFilter()) Object::shootException("The system has detected a replay on your browser window");
			$folder = "../data/".$_REQUEST['folder']."/";
...
<?php
	...
	} else if ($page == "managenullificationservices" && isset($_REQUEST['class']) && isset($_REQUEST['id']) && isset($_REQUEST['caption']) && isset($_REQUEST['folder']) && isset($_REQUEST['key']) && Authorize::isAllowable($config, "manage".strtolower($_REQUEST['class'])."_edit", "normal", "setlog", "-1", "-1") && Authorize::isAllowable($config, "manageresults_nulify", "normal", "setlog", "-1", "-1"))	{
		$conn = mysql_connect($hostname, $user, $pass) or die("Could not connect to a database services");
		$promise1 = new Promise();
		$promise1->setPromise(true);
		$object1 = null;
...
<?php
		...
		if (! is_null($briefcaseList) && $blnBriefcaseManager)	{
			foreach ($briefcaseList as $briefcaseId)	{
				...
				if ($briefcase1->canDownloadNow())	{
					$filename = "../data/briefcase/".$briefcase1->getRawMarksCSVFile();
					if (! is_null($briefcase1->getRawMarksCSVFile()) && file_exists($filename))	{
						$actionIcons .= "<a href=\"$filename\" title=\"Download Results\" class=\"ui-sys-icon-80-by-16 icon-download\"></a>";
					}
				}
